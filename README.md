...
- **Piece themes.** Five Lichess SVG piece sets (`cburnett`, `merida`, `alpha`,
  `california`, `gioco`) live under `web/public/pieces/`. The board picks a set
  via a `.theme-<set>` class (rules in `web/src/pieces.css`, generated by
  `web/scripts/gen-pieces-css.mjs`); the choice is held in a small React context
